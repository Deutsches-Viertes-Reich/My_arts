    ...
    def create_boss(boss_name):
        ...
        if boss_name == "武蔵":
            return EnemyShip("重装甲戦艦・武蔵", {
                "atk": 220, "def": 5500, "hp": 3000000, "spd": 80, "sp": 50
            }, True)

        # --- Tier: GOD (大和) - 攻防一体の究極艦 ---
        elif boss_name == "大和":
            return EnemyShip("超弩級戦艦・大和", {
                "atk": 450, "def": 3000, "hp": 2000000, "spd": 100, "sp": 50
            }, True)
        ...
        # --- Tier: SPECIAL (雪風) - 物理法則を超えたしぶとさ ---
            ...
            return EnemyShip("不沈の名駆逐艦・雪風", {
                "atk": 200, "def": 100, "hp": 9999999, "spd": 2500, "sp": 100
            }, True)

        # --- Tier: OVERKILL (赤城) - 触れたら終わりの超火力 ---
        elif boss_name == "赤城":
            return EnemyShip("第一航空戦隊・赤城", {
                "atk": 1200, "def": 800, "hp": 45000, "spd": 600, "sp": 80
            }, True)

        # --- Tier: LEGEND (長門) - 屈強なビッグセブン ---
        elif boss_name == "長門":
            return EnemyShip("七大戦艦・長門", {
                "atk": 380, "def": 2500, "hp": 70000, "spd": 150, "sp": 40
            }, True)
